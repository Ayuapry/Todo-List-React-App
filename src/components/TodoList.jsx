...
export default function todoList({ Data, setData, filter, setFilteredTodos }) {
  ...
  return (
    <div className="container bottom__container">
      ...
      <div className="">
        <button className="btn-delete md" onClick={() => removeGroupHandler("done")}>Delete Done Task</button>
        <button className="btn-delete md" onClick={() => removeGroupHandler("all")}> Delete All Task</button>
      </div>
    </div>
  );
}
